export function renderMarkdown(md: string): string {
      ...
      rows.forEach((row, i) => {
        ...
        const cls =
          i === 0
            ...
            : "border-b border-border/60 py-2 pr-3 text-ink/90 align-top";
        out.push(
  ...
  const inline = (s: string) =>
    s
      ...
      .replace(
        /`([^`]+)`/g,
        '<code class="text-accent bg-surface2 px-1.5 py-0.5 rounded text-[13px]">$1</code>'
      )
  ...
  for (const line of lines) {
    ...
    if (/^### /.test(line)) {
      flushList();
      out.push(
        `<h3 class="text-lg font-semibold mt-8 mb-2 text-ink">${inline(line.slice(4))}</h3>`
      );
    } else if (/^## /.test(line)) {
      flushList();
      out.push(
        `<h2 class="text-2xl font-semibold mt-10 mb-3 text-ink tracking-tight">${inline(line.slice(3))}</h2>`
      );
    } else if (/^# /.test(line)) {
      flushList();
      out.push(
        `<h1 class="text-3xl font-bold mb-4 text-ink tracking-tight">${inline(line.slice(2))}</h1>`
      );
