...
| **Design system** | In-app docs site: `src/design/` served at `/design` — **agents first** |
...
```
Phase 0.4  Design system site (design.academe / src/design)   ← NOW
    ↓
Phase 0.5  Marketing showcase polish (root Vite site)
    ↓
Phase 0.6  Guardrails (done — maintain in src/design content)
    ↓
Phase 0.7  UI system docs (live in src/design)
    ↓
...
|-------|-------------|-------------|
| **0.4** | Design system site + content spine | Agents use `src/design/content/`; UI at `/design` |
| **0.5** | Marketing site craft + QR + honest copy | No “all devices”; tokens B/D; QR path |
| **0.6** | Guardrails | Exists + mirrored in src/design |
| **0.7** | UI tokens/IA/screens | In src/design Identity + Product |
...
| Audience | Start |
|----------|--------|
| **Agents** | `src/design/content/00-START-HERE.md` |
| **Humans** | `pnpm dev` → http://localhost:3000/design |
| **Deep research** | `design/` (previews, scrapes, assets) |
| **Checklist** | `src/design/content/product/checklist.md` |
...
- [ ] Install + verify `pnpm dev` opens `/design`  
- [ ] Founder smoke test  
...
Details: `src/design/content/marketing/*` and earlier `design/PHASES` craft bar.

---

## Phase 0.6–0.7 — Guardrails & UI system

Mostly complete; **canonical copy lives in src/design content**.  
...
- Name: Pebby (internal)  
- States: see illustration/motion in src/design  
- [ ] Rive / WebM  
- [ ] MascotSlot in app  
...
**Full detail:** keep expanding in src/design or prior long roadmap sections as needed.
